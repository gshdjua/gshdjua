...
export default {
  ...
  methods: {
    async loadAudio() { try { const res = await request.get('/admin/public/audioList'); if (res.data.code === 200) this.audio = res.data.data.find(item => item.id == this.$route.params.id) || null } catch (e) { this.audio = null; console.error('歌曲信息加载失败:', e) } },
    ...
    async submitComment() { const content = this.commentInput.trim(); if (!content || this.commentSubmitting) return; this.commentSubmitting = true; try { const res = await request.post('/user/audio/' + this.$route.params.id + '/comments', { content }); if (res.data.code === 200) { this.commentInput = ''; await this.loadComments() } else alert(res.data.msg || '评论发布失败') } catch (e) { alert(e.response?.data?.msg || '评论发布失败，请稍后重试') } finally { this.commentSubmitting = false } },
    async toggleCommentLike(comment) { try { const res = await request.post('/user/comments/' + comment.id + '/like/toggle'); if (res.data.code === 200) { comment.liked = res.data.data.liked; comment.likeCount = res.data.data.likeCount; if (this.commentSort === 'hot') this.loadComments() } else alert(res.data.msg || '点赞操作失败') } catch (e) { alert(e.response?.data?.msg || '点赞操作失败，请稍后重试') } },
    async deleteComment(comment) { if (!window.confirm('确定删除这条评论吗？')) return; try { const res = await request.delete('/user/comments/' + comment.id); if (res.data.code === 200) this.comments = this.comments.filter(item => item.id !== comment.id); else alert(res.data.msg || '评论删除失败') } catch (e) { alert(e.response?.data?.msg || '评论删除失败，请稍后重试') } },
